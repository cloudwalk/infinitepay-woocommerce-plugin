...
(function ($) {
  ...
  $(function () {
    ...
    function clearErrors() {
      for (let i = 0; i < document.querySelectorAll('[data-checkout]').length; i++) {
        const errorElement = document.querySelectorAll('[data-checkout]')[i];
        errorElement.classList.remove('ip-form-control-error');
      }

      for (let j = 0; j < document.querySelectorAll('.ip-error').length; j++) {
    ...
    function validateMainInputs() {
      ...
      const mainInputs = ['installments', 'cardSecurityCode', 'cardExpirationDate', 'cardNumber', 'docNumber'];
      ...
      for (let k = 0; k < formInputs.length; k++) {
        ...
        if (mainInputs.indexOf(element.getAttribute('data-checkout')) > -1) {
          ...
          if (withError) {
            const errorSpan = form.querySelectorAll('span[data-main="#' + element.id + '"]');
